...
ylabel('North');
xlabel('East');
title('Simulation 10 - ellipsoid with integral action')
...
xlabel('Time[s]');
title('Simulation 10 - Commanded control input to each thruster, with integral action')
subplot(2,1,2)
...
xlabel('Time[s]');
title('Simulation 10 - Reference forces from controller, with integral action ')
...
set(hleg, 'Interpreter','Latex');
ylabel('N [m]');
title('Sim 10 - NED positions \eta')
...
title('Sim 10 - Measurements are available vs ship in dead reckoning')
ylabel('Yes/No')
xlabel('Time(s)')
...
xlabel('Time (s)')
title('Sim 10 - Bias Estimate as a function of time with user defined inputs')
