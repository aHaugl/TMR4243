...
ylabel('North');
xlabel('East');
title('Simulation 7 - straight line without integral action')
...
xlabel('Time[s]');
title('Simulation 7 - Commanded control input to each thruster, without integral action')
subplot(2,1,2)
...
xlabel('Time[s]');
title('Simulation 7 - Reference forces from controller, without integral action ')
...
hleg = legend('$x_{real}$','$x_{meas}$','$\hat{x}$','$\tilde{x}$','Location', 'northwest');
set(hleg, 'Interpreter','Latex');
ylabel('x [m]');
title('Sim 7 - NED positions \eta')
...
hleg = legend('$y_{real}$','$y_{meas}$','$\hat{y}$','$\tilde{y}$','Location', 'northwest');
set(hleg, 'Interpreter','Latex');
ylabel('y [m]');
...
title('Sim 7 - Measurements are available vs ship in dead reckoning')
ylabel('Yes/No')
xlabel('Time(s)')
...
xlabel('Time (s)')
title('Sim 7 - Bias Estimate as a function of time with user defined inputs')
